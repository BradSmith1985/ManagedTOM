...
// Throws an exception for the specified HRESULT (unless S_OK)
#define THROW_HRESULT(hr) if (FAILED(hr)) Marshal::ThrowExceptionForHR(hr)

// Throws NotSupportedException
#define THROW_TOM2_ONLY() throw gcnew NotSupportedException("This operation is not supported in the Text Object Model (TOM) version 1.")

// Throws NotSupportedException if interface pointer 'x' is NULL
#define THROW_IF_NOT_INTERFACE(x) if (x == NULL) THROW_TOM2_ONLY()

// Converts tomBool value to Nullable<bool>
#define LONG_TO_NBOOL(x) (x == tomUndefined) ? Nullable<bool>() : (Nullable<bool>)(x == tomTrue)

// Converts Nullable<bool> value to tomBool
#define NBOOL_TO_LONG(x) x.HasValue ? (x.Value ? tomTrue : tomFalse) : tomUndefined

#define NAMESPACE_DECL TextObjectModel
#define NAMESPACE_CLOSE /* END */
#define NAMESPACE TextObjectModel

namespace NAMESPACE {

	// Newer version of tomConstants (from WWW)
